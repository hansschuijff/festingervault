...
export default function FilterSheet({ collection }: Props) {
  ...
  const setFilter = useCallback(
    (key: string, values: string[]) => {
      setIntentFilter(prev => removeEmptyParams({
        ...prev,
        [key]: values,
      }));
    },
  ...
  const openChange = (open: boolean) => {
	...
	const enabledFilters=collection.options
    ...
    <Sheet onOpenChange={openChange}>
      ...
      <SheetContent className="overflow-y-auto">
        <SheetHeader>
          ...
          <SheetDescription>
            {__("Select Filters to fine tune your search results.")}
          </SheetDescription>
        </SheetHeader>
        <div className="py-4">
          {collection.options && (
            <Accordion type="single" collapsible defaultValue={enabledFilters[0]?.id} className="w-full">
