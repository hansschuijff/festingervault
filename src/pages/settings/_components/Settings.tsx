...
export default function SettingsForm({ setting }: Props) {
	...
	const { data: roles } = useApiFetch<Record<string, string>>("setting/roles");
	const { mutateAsync } =
	...
	return (
		<>
						...
						>
							<Switch
								...
								onCheckedChange={checked =>
									setSetting(prev => ({ ...prev, autoactivate: checked }))
								}
							/>
						</SettingControl>
						{roles && (
							...
							>
								<div className="flex flex-col gap-2">
									{Object.entries(roles).map(([role, label]) => (
										<div className="flex flex-row gap-2" key={role}>
											<Switch
												checked={settings.roles?.includes(role)}
												onCheckedChange={checked =>
													setSetting(prev => {
														const _roles = new Set(prev.roles);
														if (checked) {
															_roles.add(role);
														} else {
															_roles.delete(role);
														}
														return { ...prev, roles: Array.from(_roles) };
													})
												}
											/>
											{label}
			...
			<Card>
				<CardHeader className="border-b">{__("Advanced Settings")}</CardHeader>
				<CardContent>
					<div className="flex flex-col gap-2">
						<SettingControl
						...
						>
							<Switch
								...
								onCheckedChange={checked =>
									setSetting(prev => ({ ...prev, clean_on_uninstall: checked }))
								}
							/>
						</SettingControl>
					</div>
