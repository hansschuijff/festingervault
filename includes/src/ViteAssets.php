...
class ViteAssets {
    ...
    public function register(string $entry, array $options): ?array {
        ...
        $options["script-dependencies"] = array_merge($entry_dependencies, $options["script-dependencies"]);
    ...
    protected function init_dependencies() {
        $this->dependencies = $this->read_json_file($this->config["dependencies"]);
    }
    ...
    protected function register_for_dev(string $entry, array $options): ?array {
        ...
        $dependencies = apply_filters("dependencies/".$options['handle'],array_merge([$this->get_dev_client_handle()], $options['script-dependencies']));
    ...
    protected function register_for_prod(string $entry, array $options): ?array {
        ...
        if (!$options['skip-script']) {
            ...
            $this->load_script_as_module($options['handle']);
            $dependencies=apply_filters("dependencies/".$options['handle'],$options['script-dependencies']);
